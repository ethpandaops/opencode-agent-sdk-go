#!/usr/bin/env bash
#
# test_examples.sh — Run SDK examples and verify output using opencode CLI.
#
# Discovers runnable examples in examples/*, runs each one, then asks
# opencode to judge whether the output matches the source code's intended
# behavior.
#
...
# Parse flags
# ---------------------------------------------------------------------------
while getopts "n:t:o:s:f:m:a:kh" opt; do
	case "$opt" in
	n) PARALLEL="$OPTARG" ;;
	t) TIMEOUT="$OPTARG" ;;
	o) OUTDIR="$OPTARG" ;;
...
# Scratch dir for opencode verifier runs so they don't create sessions
# or files inside the repo.
VERIFY_DIR="$OUTDIR/verifier-cwd"
mkdir -p "$VERIFY_DIR"
...
echo "Verify agent: $VERIFY_AGENT"
...
example_args() {
	...
	# No opencode-sdk example currently takes positional args; kept here
	# so future examples that do (e.g. `./example all`) can opt in.
	case "$name" in
	*)
		echo ""
		;;
...
# ---------------------------------------------------------------------------
# Parse an `opencode run --format json` result file into verdict + reason.
# The stream is newline-delimited JSON events; the final assistant text
# lives in `.part.text` on the last `text`-typed event.
# Writes: <name>.verdict ("true"/"false") and <name>.reason
# ---------------------------------------------------------------------------
parse_result() {
	local name="$1"
	...
	if [[ ! -s "$result_file" ]]; then
		...
		echo "Opencode verification failed (empty result)" >"$OUTDIR/${name}.reason"
		return
	fi

	local raw inner verdict reason

	raw="$(jq -r 'select(.type=="text") | .part.text // empty' "$result_file" \
		2>/dev/null | tail -n 1)"

	if [[ -z "$raw" ]]; then
		echo "false" >"$OUTDIR/${name}.verdict"
		echo "Opencode verification failed (no text event found)" >"$OUTDIR/${name}.reason"
		return
	fi

	# Handle fallback formats if JSON is wrapped in markdown fences or prose.
	...
	if ! echo "$inner" | jq -e . >/dev/null 2>&1; then
		echo "false" >"$OUTDIR/${name}.verdict"
		echo "Opencode verification failed (invalid JSON payload)" >"$OUTDIR/${name}.reason"
		return
	fi
...
process_example() {
	...
	prompt="$(cat <<PROMPT
Below is the Go source code for an opencode-agent-sdk-go example named "$name" and its output log.
